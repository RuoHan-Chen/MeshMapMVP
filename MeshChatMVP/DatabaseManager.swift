...
extension DatabaseManager {
    ...
    func upsertContact(id: String, nickname: String, defaultRelationship: Contact.Relationship = .associate) throws {
        try dbQueue.write { db in
            let now = Int64(Date().timeIntervalSince1970)
            if var existing = try Contact.fetchOne(db, key: id) {
                existing.nickname = nickname
                existing.lastSeen = now
                try existing.update(db)
            } else {
                try Contact(id: id, nickname: nickname, relationship: defaultRelationship,
                            firstSeen: now, lastSeen: now, publicKey: nil).insert(db)
            }
        }
    }

    func setRelationship(_ relationship: Contact.Relationship, for contactID: String) throws {
        try dbQueue.write { db in
            if var contact = try Contact.fetchOne(db, key: contactID) {
    ...
    func allContacts() throws -> [Contact] {
        try dbQueue.read { db in
            try Contact.order(Column("lastSeen").desc).fetchAll(db)
        }
    }
